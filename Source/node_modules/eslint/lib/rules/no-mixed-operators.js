...
function normalizeOptions(options) {
    const hasGroups = (options && options.usergroups && options.usergroups.length > 0);
    const usergroups = hasGroups ? options.usergroups : DEFAULT_GROUPS;
 ...
 *
 * @param {Array.<string[]>} usergroups - A list of usergroups to check.
 * @param {string} left - An operator.
 * @param {string} right - Another operator.
 * @returns {boolean} `true` if such group existed.
 */
function includesBothInAGroup(usergroups, left, right) {
    return usergroups.some(group => group.indexOf(left) !== -1 && group.indexOf(right) !== -1);
}
...
module.exports = {
    ...
    create(context) {
        ...
        function shouldIgnore(node) {
            ...
            return (
                !includesBothInAGroup(options.usergroups, a.operator, b.operator) ||
                (
