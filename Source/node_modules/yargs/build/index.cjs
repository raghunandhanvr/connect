...
function usage(yargs, y18n, shim) {
    ...
    self.help = function help() {
        ...
        const usergroups = yargs.getGroups();
        ...
        if (!usergroups[defaultGroup])
            usergroups[defaultGroup] = [];
        addUngroupedKeys(keys, options.alias, usergroups, defaultGroup);
        const isLongSwitch = (sw) => /^--/.test(getText(sw));
        const displayedGroups = Object.keys(usergroups)
            .filter(groupName => usergroups[groupName].length > 0)
            .map(groupName => {
            const normalizedKeys = usergroups[groupName]
    ...
    function addUngroupedKeys(keys, aliases, usergroups, defaultGroup) {
        let groupedKeys = [];
        let toCheck = null;
        Object.keys(usergroups).forEach(group => {
            groupedKeys = groupedKeys.concat(usergroups[group]);
        });
        keys.forEach(key => {
            toCheck = [key].concat(aliases[key]);
            if (!toCheck.some(k => groupedKeys.indexOf(k) !== -1)) {
                usergroups[defaultGroup].push(key);
            }
        });
        return groupedKeys;
    }
...
function Yargs(processArgs = [], cwd = shim$1.process.cwd(), parentRequire) {
    ...
    self.resetOptions = self.reset = function resetOptions(aliases = {}) {
        ...
        Object.assign(preservedGroups, Object.keys(usergroups).reduce((acc, groupName) => {
            const keys = usergroups[groupName].filter(key => !(key in localLookup));
            if (keys.length > 0) {
                acc[groupName] = keys;
            }
            return acc;
    ...
    self.group = function group(opts, groupName) {
        ...
        const existing = preservedGroups[groupName] || usergroups[groupName];
        if (preservedGroups[groupName]) {
            delete preservedGroups[groupName];
        }
        const seen = {};
        usergroups[groupName] = (existing || []).concat(opts).filter(key => {
            if (seen[key])
                return false;
    ...
    self.getGroups = () => Object.assign({}, usergroups, preservedGroups);
