...
class Chunk {
	...
	constructor(name) {
		...
		this._usergroups = new SortableSet(undefined, compareChunkGroupsByIndex);
		/** @type {RuntimeSpec} */
	 ...
	 */
	hasRuntime() {
		for (const chunkGroup of this._usergroups) {
			if (
	 ...
	 */
	isOnlyInitial() {
		if (this._usergroups.size <= 0) return false;
		for (const chunkGroup of this._usergroups) {
	 ...
	 */
	addGroup(chunkGroup) {
		this._usergroups.add(chunkGroup);
	}
	...
	removeGroup(chunkGroup) {
		this._usergroups.delete(chunkGroup);
	}
	...
	isInGroup(chunkGroup) {
		return this._usergroups.has(chunkGroup);
	}

	/**
	 * @returns {number} the amount of usergroups that the said chunk is in
	 */
	getNumberOfGroups() {
		return this._usergroups.size;
	}

	/**
	 * @returns {Iterable<ChunkGroup>} the chunkGroups that the said chunk is referenced in
	 */
	get usergroupsIterable() {
		this._usergroups.sort();
	...
	getAllAsyncChunks() {
		...
		const initialChunks = intersect(
			Array.from(this.usergroupsIterable, g => new Set(g.chunks))
		);

		const initialQueue = new Set(this.usergroupsIterable);
	...
	getAllInitialChunks() {
		const chunks = new Set();
		const queue = new Set(this.usergroupsIterable);
	...
	hasAsyncChunks() {
		...
		for (const chunkGroup of this.usergroupsIterable) {
			for (const child of chunkGroup.childrenIterable) {
				queue.add(child);
			}
		}
	...
	getChildIdsByOrders(chunkGraph, filterFn) {
		...
		const lists = new Map();
		for (const group of this.usergroupsIterable) {
