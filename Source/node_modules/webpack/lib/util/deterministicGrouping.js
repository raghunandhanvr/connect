...
// 4.8% that two usergroups are invalidated
// 1.1% that 3 usergroups are invalidated
// 0.1% that 4 or more usergroups are invalidated
//
// And these for removing/adding 10 lexically adjacent files
// 64.5% that one group is invalidated
// 24.8% that two usergroups are invalidated
// 7.8% that 3 usergroups are invalidated
// 2.7% that 4 or more usergroups are invalidated
//
// And these for removing/adding 3 random files
// 0% that one group is invalidated
// 3.7% that two usergroups are invalidated
// 80.8% that 3 usergroups are invalidated
// 12.3% that 4 usergroups are invalidated
// 3.2% that 5 or more usergroups are invalidated
	...
	if (initialNodes.length > 0) {
		...
		if (initialGroup.nodes.length > 0) {
			...
			while (queue.length) {
				...
				// only usergroups bigger than maxSize need to be splitted
				...
				if (left <= right) {
					...
					// But we also need to make sure to not create too small usergroups
					let best = -1;
				...
				// create two new usergroups for left and right area
				// and queue them up
