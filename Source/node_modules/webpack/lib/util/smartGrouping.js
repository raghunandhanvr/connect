 ...
 * @property {T} item
 * @property {Set<Group<T, R>>} usergroups
 */
...
const smartGrouping = (items, groupConfigs) => {
	...
	/**
	 * @param {Set<ItemWithGroups<T, R>>} itemsWithGroups input items with usergroups
	 * @returns {(T | R)[]} usergroups items
	 */
	const runGrouping = itemsWithGroups => {
		const totalSize = itemsWithGroups.size;
		for (const entry of itemsWithGroups) {
			for (const group of entry.usergroups) {
		...
		for (;;) {
			...
			for (const item of items) {
				...
				// Remove all usergroups that items have from the map to not select them again
				for (const group of item.usergroups) {
