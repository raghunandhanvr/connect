...
const SIMPLE_EXTRACTORS = {
			...
			if (logging || (loggingDebug && loggingDebug.length > 0)) {
				...
				for (const [origin, logEntries] of compilation.logging) {
					...
					for (const entry of logEntries) {
						...
						// Expand usergroups in verbose and debug modes
						if (
	...
	chunk: {
		...
		chunkRelations: (object, chunk, { compilation: { chunkGraph } }) => {
			...
			for (const chunkGroup of chunk.usergroupsIterable) {
		...
		chunkOrigins: (object, chunk, context, options, factory) => {
			...
			const origins = [];
			for (const g of chunk.usergroupsIterable) {
...
const spaceLimited = (itemsAndGroups, max) => {
	...
	// This are the usergroups, which take 1+ lines each
	const usergroups = itemsAndGroups.filter(c => c.children || c.filteredChildren);
	// The sizes of the usergroups are stored in groupSizes
	const groupSizes = usergroups.map(g => getItemSize(g));
	...
	let usergroupsSize = groupSizes.reduce((a, b) => a + b, 0);
	if (usergroupsSize + items.length <= max) {
		// The total size in the current state fits into the max
		// keep all
		children = usergroups.concat(items);
	} else if (
		usergroups.length > 0 &&
		usergroups.length + Math.min(1, items.length) < max
	) {
		// If each group would take 1 line the total would be below the maximum
		// collapse some usergroups, keep items
		while (usergroupsSize + items.length + (filteredChildren ? 1 : 0) > max) {
			// calculate how much we are over the size limit
			// this allows to approach the limit faster
			// it's always > 1
			const oversize =
				items.length + usergroupsSize + (filteredChildren ? 1 : 0) - max;
			...
			for (let i = 0; i < usergroups.length; i++) {
				if (groupSizes[i] === maxGroupSize) {
					...
					// current size - oversize / number of usergroups
					...
					const limited = spaceLimited(
						group.children,
						groupSizes[i] - headerSize - oversize / usergroups.length
					);
					usergroups[i] = {
						...group,
					...
					const newSize = getItemSize(usergroups[i]);
					usergroupsSize -= groupSizes[i] - newSize;
		...
		// collapse all usergroups and items
		children = usergroups.length ? collapse(usergroups) : undefined;
		filteredChildren = items.length;
	} else {
		// If we have no space
