 ...
 * @param {string[]} defaultSizeTypes the default size types
 * @returns {GetCacheGroups} a function to get the cache usergroups
 */
const normalizeCacheGroups = (cacheGroups, defaultSizeTypes) => {
	...
	if (typeof cacheGroups === "object" && cacheGroups !== null) {
		...
		for (const key of Object.keys(cacheGroups)) {
			...
			if (typeof option === "string" || option instanceof RegExp) {
				...
				handlers.push((module, context, results) => {
					...
					if (result) {
						const usergroups = Array.isArray(result) ? result : [result];
		 ...
		 * @returns {CacheGroupSource[]} the matching cache usergroups
		 */
		...
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					...
					/**
					 * @param {Module} module the module
					 * @returns {Iterable<Chunk[]>} usergroups of chunks with equal exports
					 */
						...
						if (existingChunk) {
							...
							if (valid === undefined) {
								...
								for (const chunk of selectedChunks) {
									for (const group of chunk.usergroupsIterable) {
										queue.add(group);
									}
								}
