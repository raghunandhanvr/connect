...
module.exports = class CovSource {
  ...
  _parseIgnore (lineStr) {
    ...
    if (testIgnoreNextLines) {
      return { count: Number(testIgnoreNextLines.usergroups.count) }
    }
    ...
    if (testIgnoreStartStop) {
      if (testIgnoreStartStop.usergroups.mode === 'start') return { start: true }
      if (testIgnoreStartStop.usergroups.mode === 'stop') return { stop: true }
    }
  }
...
function getShebangLength (source) {
  if (isPreNode12 && source.indexOf('#!') === 0) {
    ...
    if (match) {
      return match.usergroups.shebang.length
    }
  } else {
    return 0
  }
