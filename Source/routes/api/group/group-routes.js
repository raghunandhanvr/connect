...
app.post('/create-group', async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    if (affectedRows == 1) {
      await makeDir(`${root}/dist/usergroups/${insertId}`)
      createReadStream(`${root}/dist/images/wheel.jpg`).pipe(
        createWriteStream(`${root}/dist/usergroups/${insertId}/avatar.jpg`)
      )
...
app.post('/edit-group', async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE usergroups SET name=?, bio=?, group_type=? WHERE group_id=?',
...
app.post('/is-group-valid', async (req, res) => {
  let [{ groupCount }] = await db.query(
    'SELECT COUNT(group_id) AS groupCount FROM usergroups WHERE group_id=? LIMIT 1',
    [req.body.grp_id]
  )
...
app.post('/get-group-details', async (req, res) => {
  let { grp_id } = req.body,
    _details = await db.query(
      'SELECT usergroups.group_id, usergroups.name, usergroups.bio, usergroups.admin, users.username AS admin_username, usergroups.group_type, usergroups.created FROM usergroups, users WHERE usergroups.group_id=? AND usergroups.admin = users.id',
      [grp_id]
    ),
...
app.post('/change-admin', async (req, res) => {
  try {
    ...
    await db.query('UPDATE usergroups SET admin=? WHERE group_id=?', [user, group])
    res.json({
      success: true,
