...
// SEARCH CONNECT [REQ = VALUE]
app.post('/search-connect', async (req, res) => {
  let { value } = req.body,
    { id } = req.session,
    ...
    users = [],
    _usergroups = await db.query(
      `SELECT group_id, name FROM usergroups WHERE name LIKE "%${value}%" ORDER BY group_id DESC LIMIT 7`
    ),
    usergroups = [],
  ...
  res.json({ users, usergroups, hashtags })
})
