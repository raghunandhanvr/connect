...
// GROUPS TO EXPLORE
app.post('/get-usergroups-to-explore', async (req, res) => {
  let { id } = req.session,
    _usergroups = await db.query(
      'SELECT group_id, name, admin, created FROM usergroups ORDER BY RAND()'
    ),
    usergroups = []
  ...
  let orderByMutualMembers = _.orderBy(usergroups, ['mutualMembersCount'], ['desc'])
