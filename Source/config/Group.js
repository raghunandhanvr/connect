...
const getWhatOfGrp = async (what, group) => {
  let s = await db.query(`SELECT ${what} FROM usergroups WHERE group_id=?`, [group])
...
const deleteGroup = async group => {
  ...
  await db.query('DELETE FROM usergroups WHERE group_id=?', [group])

  DeleteAllOfFolder(`${root}/dist/usergroups/${group}/`)
  await dltDir(`${root}/dist/usergroups/${group}`)
}
